...
def build_digest(df_out: pd.DataFrame, max_rows=20) -> str:
	use = df_out.head(max_rows)
	lines = []
	for _, r in use.iterrows():
		lines.append(
			f"新聞: {r['input_text']}\n情緒: {r['sentiment']} (conf={r['confidence']:.4f})\n"
		)
	return "\n".join(lines)
	...
	if uploaded is not None:
		...
		if missing:
			...
			if st.button("Run batch sentiment + summary", type="primary"):
				...
				st.dataframe(out_df[["title", "text", "sentiment", "confidence"]].head(20), use_container_width=True)
				...
				digest = build_digest(out_df, max_rows=min(20, len(out_df)))
				summary = run_p2(sum_pipe, digest)
